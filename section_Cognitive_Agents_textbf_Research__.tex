...
\subsection{Knowledge Representation Interface (KRI)}
...
\textbf{Deliverables}
\begin{itemize}
\item GOAL Agents capable of using ontological technology for their Knowledge Representation (OWL with rules), called OWL Agents
\item OWL Agents in a multi-agent system that operate on the same ontology as their knowledge, and share a belief base among each other (a.k.a have a common belief base), while having their private mental bases as well
\item A logic to decide which information to be shared with all other agents in the system (put in the shared mental base), and which to be kept private for an agent
\item A GOAL Environment representing the TRADR display system, which is the means of performing actions for agents
\item OWL Agents capable of using the Semantic Web to acquire external information that is needed, such as weather data, topographical, geographical data, etc...
\end{itemize}
